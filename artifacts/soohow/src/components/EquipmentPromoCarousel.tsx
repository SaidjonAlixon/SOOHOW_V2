  ...
  return (
    ...
    >
      ...
      <div className="relative z-0 flex-1 min-h-[260px] bg-white dark:bg-slate-100">
        ...
        <motion.div
          key={`progress-${index}`}
          className="absolute bottom-0 left-0 z-[2] h-1 bg-[#00A8E8]"
          initial={{ width: "0%" }}
          animate={{ width: "100%" }}
        ...
        {count > 1 && (
          <div
            className="absolute bottom-4 left-0 right-0 z-20 flex justify-center gap-2 px-4"
            aria-hidden
          >
            {equipmentPromoSlides.map((s, i) => (
              <button
                ...
                className={cn(
                  ...
                  i === index
                    ? "w-8 bg-[#00A8E8]"
                    : "w-2 bg-white/90 dark:bg-slate-800/90 ring-1 ring-black/10 hover:bg-[#00A8E8]/80",
                )}
                aria-label={`${i + 1}`}
              />
            ))}
      ...
      <div className="relative z-10 border-t site-border/60 bg-[hsl(var(--site-card)/0.95)] backdrop-blur-sm p-4 md:p-5">
        <AnimatePresence mode="wait">
          ...
          >
            <p className="text-[10px] font-sans uppercase tracking-widest site-muted mb-2 leading-relaxed">
              {product.category}
            </p>
