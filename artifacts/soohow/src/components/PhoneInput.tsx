...
export function PhoneInput({ value, onChange, onBlur, className, id }: PhoneInputProps) {
  ...
  return (
    <div
      className={cn(
        "flex h-[50px] w-full rounded-md border site-border site-section focus-within:ring-1 focus-within:ring-[#00A8E8]/50",
        className,
      )}
    >
      <DropdownMenu open={open} onOpenChange={setOpen} modal={false}>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            className="relative z-10 flex h-full shrink-0 items-center gap-1.5 border-r site-border px-2.5 sm:px-3 text-sm site-heading hover:bg-white/5 transition-colors cursor-pointer"
            aria-label={t("phoneCountries.select")}
          >
            <span className="text-base font-semibold uppercase tracking-wide text-[#00A8E8]">
              {country.id}
            </span>
            <span className="font-mono text-xs sm:text-sm text-[hsl(var(--site-fg)/0.85)]">
              {country.dialCode}
            </span>
            <ChevronDown size={14} className="text-[#00A8E8] opacity-80 shrink-0" />
          </button>
        </DropdownMenuTrigger>
        <DropdownMenuContent
          align="start"
